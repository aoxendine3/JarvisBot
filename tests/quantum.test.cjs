...
const integritySentry = require('../lib/core/quantum_sentry.cjs');

console.log("--- TESTING COMPOSITE INTEGRITY SENTRY ---");
...
const manifest = integritySentry.buildManifest(testDir);
console.log(`✅ Manifest Root Hash: ${manifest.rootHash.slice(0, 32)}...`);
console.log(`✅ Total Nodes Attested: ${manifest.totalNodes}`);

const verifyClean = integritySentry.verifyManifest(testDir, manifest);
...
const verifyTampered = integritySentry.verifyManifest(testDir, manifest);
...
console.log("--- COMPOSITE SENTRY VERIFICATION COMPLETE ---");
